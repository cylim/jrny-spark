...
for (let g = 0; g < GAMES; g++) {
  ...
  while (state.phase !== "finished" && guard++ < 2000) {
    if (state.phase === "awaitRoll") {
      ...
      state = applyEvent(CLASSIC, state, {
        type: rng() < 0.5 ? "SNAKE_CHARM" : "SNAKE_ACCEPT",
      });
  ...
  if (state.phase !== "finished")
    throw new Error(`game ${g}: did not terminate (dead end?)`);
  results.push({
...
const sorted = (key: "rolls" | "cards") =>
  results.map((r) => r[key]).sort((a, b) => a - b);
...
console.log(
  `rolls   p10/p50/p90: ${pct(rolls, 0.1)} / ${pct(rolls, 0.5)} / ${pct(rolls, 0.9)}`
);
console.log(
  `cards   p10/p50/p90: ${pct(cards, 0.1)} / ${pct(cards, 0.5)} / ${pct(cards, 0.9)}`
);
console.log(
...
console.log(
  `(target: median 15–30 min, PRD §2.3 — tune board/neutral density if outside)`
);
