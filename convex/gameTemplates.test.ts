...
test("save rejects a name over 80 characters", async () => {
  ...
  await expect(
    asAlex.mutation(api.gameTemplates.save, { ...TEMPLATE, name: "n".repeat(81) }),
  ).rejects.toThrow(/80/);
});
...
test("save rejects more than 100 pinned prompts", async () => {
  ...
  await expect(
    asAlex.mutation(api.gameTemplates.save, { ...TEMPLATE, customPrompts: pins }),
  ).rejects.toThrow(/100/);
});
...
test("save accepts values exactly at the caps", async () => {
  ...
  const pins = Array.from({ length: 100 }, (_, i) => pin(i + 2, "x".repeat(280)));
  await asAlex.mutation(api.gameTemplates.save, {
    ...TEMPLATE,
    name: "n".repeat(80),
    customPrompts: pins,
...
test("a user's 51st Template is rejected, but editing at the cap still works", async () => {
  ...
  await expect(asAlex.mutation(api.gameTemplates.save, TEMPLATE)).rejects.toThrow(/50/);
...
test("another user cannot update or delete someone else's Template", async () => {
  ...
  await expect(
    asBlair.mutation(api.gameTemplates.save, { ...TEMPLATE, id, name: "Hijacked" }),
  ).rejects.toThrow(/not found/i);
  await expect(asBlair.mutation(api.gameTemplates.remove, { id })).rejects.toThrow(
    /not found/i,
  );
...
test("save against an unknown deck is rejected", async () => {
  ...
  await expect(
    asAlex.mutation(api.gameTemplates.save, { ...TEMPLATE, deckSlug: "no-such-deck" }),
...
test("save against an inactive deck is rejected", async () => {
  ...
  await expect(asAlex.mutation(api.gameTemplates.save, TEMPLATE)).rejects.toThrow(
